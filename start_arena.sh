#!/bin/bash
# Start-Script für Arena Voting System
# Startet alle benötigten Services mit einem Befehl

set -e

echo "🚀 Arena Voting Startup"
echo "================================="
...
# Start API
echo -e "\n${YELLOW}2. Starte Arena-API (Port 8001)...${NC}"
export KEY_VAULT_NAME="kicwa-keyvault-lab"
/Users/browse/.pyenv/versions/3.11.7/bin/python -m uvicorn src.arena.openwebui_api_llm:app --host 0.0.0.0 --port 8001 > /tmp/llm_api.log 2>&1 &
API_PID=$!
sleep 3
echo -e "${GREEN}✓ API gestartet (PID: $API_PID)${NC}"

# Start Voting UI
echo -e "${YELLOW}3. Starte Voting UI (Port 8002)...${NC}"
/Users/browse/.pyenv/versions/3.11.7/bin/python -m uvicorn src.arena.voting_ui_simple:app --host 0.0.0.0 --port 8002 > /tmp/voting_ui.log 2>&1 &
VOTING_PID=$!
sleep 2
...
echo ""
echo "📖 Dokumentation: src/arena/VOTING.md"
echo ""
echo "🛑 Zum Stoppen: pkill -f 'uvicorn'"
echo ""
